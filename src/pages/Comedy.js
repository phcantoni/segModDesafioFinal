...
function Comedy({showLink = true}) {

    const [itens, setItens] = useState([]);

    fetch(`https://api.themoviedb.org/3/discover/movie?with_genres=35&language=pt-BR&api_key=${API_KEY}`)
            .then(response => response.json())
            .then(data => setItens(data.results))


    useEffect(() => {
        const fecthing = "https://api.themoviedb.org/3/discover/movie?with_genres=35&language=pt-BR&api_key=${API_KEY}";
